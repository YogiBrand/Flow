...
) AS new_values (name, type, description, icon_name, category, input_schema, output_schema, default_config)
WHERE NOT EXISTS (
    SELECT 1 FROM tool_templates WHERE name = new_values.name
);
...
) AS new_values (name, platform_key, description, icon_emoji, api_base_url, auth_type, rate_limits, supported_features, configuration_schema)
WHERE NOT EXISTS (
    SELECT 1 FROM platform_templates WHERE platform_key = new_values.platform_key
);
...
) AS new_values (name, category, description, content_type, content, metadata, tags)
WHERE NOT EXISTS (
    SELECT 1 FROM knowledge_base_templates WHERE name = new_values.name
);

-- Insert more Knowledge Base Templates (only if they don't exist)
INSERT INTO knowledge_base_templates (name, category, description, content_type, content, metadata, tags)
SELECT * FROM (VALUES
...
) AS new_values (name, type, description, content, category, metadata, tags)
WHERE NOT EXISTS (
    SELECT 1 FROM resource_templates WHERE name = new_values.name
);

-- Insert more Resource Templates (only if they don't exist)
INSERT INTO resource_templates (name, type, description, content, category, metadata, tags)
SELECT * FROM (VALUES
