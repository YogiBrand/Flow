...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'tool_templates' 
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'platform_templates' 
    AND rowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'knowledge_base_templates' 
    AND rowsecurity = true
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_tables 
    WHERE tablename = 'resource_templates' 
    AND rowsecurity = true
  ) THEN
...
DROP POLICY IF EXISTS "Anyone can read tool templates" ON tool_templates;
DROP POLICY IF EXISTS "Anyone can read platform templates" ON platform_templates;
DROP POLICY IF EXISTS "Anyone can read public knowledge base templates" ON knowledge_base_templates;
DROP POLICY IF EXISTS "Anyone can read public resource templates" ON resource_templates;

-- Create RLS Policies (public read access for templates)
...
) AS v(name, type, description, icon_name, category, input_schema, output_schema, default_config)
WHERE NOT EXISTS (SELECT 1 FROM tool_templates WHERE tool_templates.name = v.name);
...
) AS v(name, platform_key, description, icon_emoji, api_base_url, auth_type, rate_limits, supported_features, configuration_schema)
WHERE NOT EXISTS (SELECT 1 FROM platform_templates WHERE platform_templates.platform_key = v.platform_key);
...
5. Lead Qualification and Nurturing
 ...
 '{"focus": "lead_generation", "last_updated": "2024-01-12", "difficulty": "advanced"}'::jsonb,
 ARRAY['leads', 'generation', 'nurturing', 'qualification', 'conversion']),
...
5. Continuous Improvement
 ...
 '{"department": "support", "last_updated": "2024-01-08", "difficulty": "beginner"}'::jsonb,
 ARRAY['support', 'customer_service', 'communication', 'processes', 'metrics'])
) AS v(name, category, description, content_type, content, metadata, tags)
WHERE NOT EXISTS (SELECT 1 FROM knowledge_base_templates WHERE knowledge_base_templates.name = v.name);
...
Hashtags: #ContentMarketing #SocialMedia #MarketingTips #Engagement #DigitalMarketing',
 'social_media', '{"platforms": ["instagram", "linkedin", "twitter"], "engagement_rate": "high"}'::jsonb,
...
- Match subject line to email content',
 'email_marketing', '{"open_rate_improvement": "25-40%", "tested": true}'::jsonb,
...
7. Voice Checklist
 ...
 'branding', '{"completeness": "comprehensive", "usage": "internal_external"}'::jsonb,
 ARRAY['brand', 'voice', 'guidelines', 'communication', 'style']),
...
- Conversion rates
- Follower growth',
 'planning', '{"frequency": "monthly", "platforms": "multi", "metrics_included": true}'::jsonb,
 ARRAY['calendar', 'planning', 'content', 'scheduling', 'strategy']),
...
7. Promotion Strategy
 ...
 'lead_generation', '{"conversion_rate": "15-25%", "follow_up_included": true}'::jsonb,
 ARRAY['lead_magnet', 'conversion', 'template', 'marketing', 'funnel'])
) AS v(name, type, description, content, category, metadata, tags)
WHERE NOT EXISTS (SELECT 1 FROM resource_templates WHERE resource_templates.name = v.name);
