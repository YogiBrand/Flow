/*
  ...
  1. New Tables
    - `ai_agents` - Core agent information
    - `agent_knowledge_bases` - Knowledge storage for agents
    - `agent_tools` - Tools configuration for agents
    - `agent_prompts` - System and custom prompts
    - `agent_triggers` - Trigger configurations
    - `agent_escalations` - Escalation rules
    - `agent_variables` - Custom variables
    - `agent_metadata` - Additional metadata

  2. Security
    - Enable RLS on all tables
    - Add policies for user access control
    - Link everything to user_id for proper isolation

  3. Relationships
    - All agent-related tables link to ai_agents.id
    - ai_agents links to users.id for ownership
*/
...
CREATE TRIGGER update_agent_knowledge_bases_updated_at BEFORE UPDATE ON agent_knowledge_bases FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
...
CREATE TRIGGER update_agent_triggers_updated_at BEFORE UPDATE ON agent_triggers FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_agent_escalations_updated_at BEFORE UPDATE ON agent_escalations FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_agent_variables_updated_at BEFORE UPDATE ON agent_variables FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
CREATE TRIGGER update_agent_metadata_updated_at BEFORE UPDATE ON agent_metadata FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();