...
DO $$
BEGIN
  -- Drop and recreate policies for tool_templates
  ...
  -- Drop and recreate policies for platform_templates
  ...
  -- Drop and recreate policies for knowledge_base_templates
  ...
  -- Drop and recreate policies for resource_templates
...
-- Insert Tool Templates only if table is empty
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM tool_templates LIMIT 1) THEN
...
-- Insert Platform Templates only if table is empty
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM platform_templates LIMIT 1) THEN
...
-- Insert Knowledge Base Templates only if table is empty
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM knowledge_base_templates LIMIT 1) THEN
...
-- Insert Resource Templates only if table is empty
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM resource_templates LIMIT 1) THEN
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_tool_templates_updated_at') THEN
    CREATE TRIGGER update_tool_templates_updated_at 
      BEFORE UPDATE ON tool_templates 
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_platform_templates_updated_at') THEN
    CREATE TRIGGER update_platform_templates_updated_at 
      BEFORE UPDATE ON platform_templates 
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_knowledge_base_templates_updated_at') THEN
    CREATE TRIGGER update_knowledge_base_templates_updated_at 
      BEFORE UPDATE ON knowledge_base_templates 
      FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();
  END IF;

  IF NOT EXISTS (SELECT 1 FROM pg_trigger WHERE tgname = 'update_resource_templates_updated_at') THEN
