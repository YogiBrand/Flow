...
const AIAgentBuilderNew: React.FC<AIAgentBuilderNewProps> = ({ agent, onBack }) => {
  ...
  const addTool = async (toolTemplate: any) => {
      ...
      {currentAgent.tools.length === 0 ? (
        ...
        <div className="space-y-3">
          {currentAgent.tools.map((tool) => {
            const toolTemplate = availableTools.find(t => t.type === tool.type);
            const IconComponent = toolTemplate?.icon || Zap;
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">{toolTemplate?.description || tool.type.replace('_', ' ')}</p>
                  </div>
                </div>
