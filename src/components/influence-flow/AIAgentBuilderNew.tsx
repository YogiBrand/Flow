...
const StepConfigModal = ({ step, onClose, onSave }: { 
  step: Tool; 
  onClose: () => void; 
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {(config.conversationHistory || []).map((msg: any, index: number) => (
                      <div key={index} className="flex gap-2">
                        <select
                          value={msg.role}
                          onChange={(e) => {
                            const newHistory = [...(config.conversationHistory || [])];
                            newHistory[index] = { ...msg, role: e.target.value };
                            setConfig({ ...config, conversationHistory: newHistory });
                          }}
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const newHistory = [...(config.conversationHistory || [])];
                            newHistory[index] = { ...msg, content: e.target.value };
                            setConfig({ ...config, conversationHistory: newHistory });
                          }}
                        ...
                        <button
                          onClick={() => {
                            const newHistory = (config.conversationHistory || []).filter((_: any, i: number) => i !== index);
                            setConfig({ ...config, conversationHistory: newHistory });
                          }}
                    ...
                    <button
                      onClick={() => {
                        const newHistory = [...(config.conversationHistory || []), { role: 'user', content: '' }];
                        setConfig({ ...config, conversationHistory: newHistory });
                      }}
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <input
                ...
                onChange={(e) => setConfig({ ...config, numResults: parseInt(e.target.value) })}
                className="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                min="1"
                max="50"
              />
          ...
          <div className="space-y-6">
            <div>
              ...
              >
                ...
                <option value="kb1">Marketing Knowledge</option>
                <option value="kb2">Product Documentation</option>
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">
      ...
      default:
        return (
          <div className="text-center py-8 text-gray-500">
            ...
            <p>Configuration for this step type is not yet implemented.</p>
          </div>
        );
    }
  };
...
const AIAgentBuilderNew: React.FC<AIAgentBuilderNewProps> = ({ agent, onBack }) => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <AnimatePresence>
        {showStepConfig && selectedStepForConfig && (
          <StepConfigModal
            ...
            onSave={(config) => {
              ...
              setCurrentAgent(prev => ({
                ...prev,
                tools: prev.tools.map(t => 
                  t.id === selectedStepForConfig.id ? { ...t, config } : t
                )
              }));
            }}
          />
